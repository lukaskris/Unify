 ...
 */
class TextAreaUnify(context: Context, attrs: AttributeSet) : FrameLayout(context, attrs) {
    ...
    init {
        ...
        if(!TextUtils.isEmpty(textFieldMessage)) {
    ...
    private fun configLabel(){
        ...
        editText.setOnFocusChangeListener { view, hasFocus ->
            if(hasFocus) {
                ...
                if (TextUtils.isEmpty(editText.editableText)) {
                    Handler().postDelayed({
                        textInputLayout.defaultHintTextColor = hintEmptyColorStateList
                    }, 10)
                } else {
    ...
    private fun configIcon() {
        ...
        if(!TextUtils.isEmpty(textFieldAppendUrlIcon1)) {
            setFirstIcon(textFieldAppendUrlIcon1)
        }

        if(!TextUtils.isEmpty(textFieldAppendUrlIcon2)) {
