...
import kotlinx.android.synthetic.main.activity_textfield.*
...
//        input_email.setCounter(15)
        ...
        input_disabled.isEnabled = false
        input_disabled.editText.setText("Test")
        ...
        input_name.getFirstIcon().setOnClickListener {
        ...
        input_age.editText.text = Editable.Factory.getInstance().newEditable("25")
        input_age.addOnFocusChangeListener = { _, hasFocus ->
            if(hasFocus) {
                println("input_age hasFocus")
            }
        }

        input_password.setCounter(10)

        input_price.setInputType(InputType.TYPE_CLASS_NUMBER)
        Handler().postDelayed({
            input_email.isLoading = true
        }, 5000)
        input_price.setFirstIcon(R.drawable.sample_drawable)
        input_price.setSecondIcon(R.drawable.sample_drawable2)
        input_price.getFirstIcon().setOnClickListener {
            Toast.makeText(this, "First Icon Clicked", Toast.LENGTH_SHORT).show()
        }
        input_price.getSecondIcon().setOnClickListener {
            Toast.makeText(this, "Second Icon Clicked", Toast.LENGTH_SHORT).show()
        }
        input_price.prependText("$")
        input_price.appendText("USD")
//        thousand separator from https://stackoverflow.com/a/38802287/8624588
        input_price.editText.addTextChangedListener(object : TextWatcher {
            override fun afterTextChanged(p0: Editable?) {
                input_price.editText.removeTextChangedListener(this)
                ...
                try {
                    ...
                    input_price.editText.setText(formattedString)
                    input_price.editText.setSelection(input_price.editText.text.length)
                }
                catch (nfe: NumberFormatException) {
                    nfe.printStackTrace()
                }

                input_price.editText.addTextChangedListener(this)
            }
        ...
        submit.setOnClickListener {
            var emailValue = input_email.getEditableValue().toString()
            var passwordValue = input_password.getEditableValue().toString()
            var nameValue = input_name.getEditableValue().toString()

            if(TextUtils.isEmpty(emailValue)) {
                input_email.setMessage("E-mail must be filled in")
                input_email.isInputError = true
            } else {
                if(input_email.editText.text.length < 3) {
                    input_email.isInputError = true
                    input_email.setMessage("Minimum 3 karakter")
                } else {
                    input_email.setMessage(htmlString.spannedString!!)
            ...
            if(TextUtils.isEmpty(passwordValue)) {
                input_password.setMessage("Password must be filled in")
                input_password.isInputError = true
            } else {
                input_password.setMessage("")
                input_password.isInputError = false
            }

            if(TextUtils.isEmpty(nameValue)) {
                input_name.setMessage("Name must be filled in")
                input_name.isInputError = true
            } else {
                input_name.isInputError = false
            }
        }
    }
}