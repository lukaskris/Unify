...
import id.co.app.lsdownloader.core.PriorityListProcessor
 ...
 * */
class LSDownloaderHandlerImpl(private val namespace: String,
                              private val fetchDatabaseManagerWrapper: LSDownloaderDatabaseManagerWrapper,
                              ...
                              private val fetchNotificationManager: LSDownloaderNotificationManager?,
                              private val groupInfoProvider: GroupInfoProvider,
                              private val prioritySort: PrioritySort,
                              private val createFileOnEnqueue: Boolean) : LSDownloaderHandler {

    private val listenerId = UUID.randomUUID().hashCode()
    private val listenerSet = mutableSetOf<LSDownloaderListener>()
    ...
    override fun enqueue(request: Request): Pair<Download, LSDownloaderError> {
        return enqueueRequests(listOf(request)).first()
    }

    override fun enqueue(requests: List<Request>): List<Pair<Download, LSDownloaderError>> {
    ...
    private fun enqueueRequests(requests: List<Request>): List<Pair<Download, LSDownloaderError>> {
        val results = mutableListOf<Pair<Download, LSDownloaderError>>()
        requests.forEach {
            val downloadInfo = it.toDownloadInfo(fetchDatabaseManagerWrapper.getNewDownloadInfoInstance())
            downloadInfo.namespace = namespace
            try {
                ...
                if (downloadInfo.status != Status.COMPLETED) {
                    ...
                    if (!existing) {
                        ...
                        results.add(Pair(downloadPair.first, LSDownloaderError.NONE))
                        startPriorityQueueIfNotStarted()
                    } else {
                        fetchDatabaseManagerWrapper.update(downloadInfo)
                        logger.d("Updated download $downloadInfo")
                        results.add(Pair(downloadInfo, LSDownloaderError.NONE))
                    }
                } else {
                    results.add(Pair(downloadInfo, LSDownloaderError.NONE))
                }
        ...
        return when (downloadInfo.enqueueAction) {
            EnqueueAction.UPDATE_ACCORDINGLY -> {
                if (existingDownload != null) {
                    ...
                    downloadInfo.lsDownloaderError = existingDownload.lsDownloaderError
    ...
    private fun cancelDownloads(downloads: List<DownloadInfo>): List<Download> {
        ...
        downloads.forEach {
            if (canCancelDownload(it)) {
                ...
                it.lsDownloaderError = defaultNoLSDownloaderError
    ...
    override fun resetAutoRetryAttempts(downloadId: Int, retryDownload: Boolean): Download? {
        ...
        if (download != null) {
            ...
            if (retryDownload && canRetryDownload(download)) {
                ...
                download.lsDownloaderError = defaultNoLSDownloaderError
            }
        ...
        return if (oldDownloadInfo != null) {
            if (newRequest.file == oldDownloadInfo.file) {
                ...
                if (oldDownloadInfo.status == Status.DOWNLOADING) {
                    ...
                    newDownloadInfo.lsDownloaderError = defaultNoLSDownloaderError
                } else {
                    newDownloadInfo.status = oldDownloadInfo.status
                    newDownloadInfo.lsDownloaderError = oldDownloadInfo.lsDownloaderError
                }
                ...
                Pair(enqueuePair.first, enqueuePair.second == LSDownloaderError.NONE)
            }
        } else {
    ...
    override fun getDownloadByTag(tag: String): Flow<Download?> {
        return fetchDatabaseManagerWrapper.getDownloadByTag(tag)
    }
    ...
    override fun close() {
        ...
        LSDownloaderModulesBuilder.removeNamespaceInstanceReference(namespace)
    }
    ...
    override fun addListener(listener: LSDownloaderListener, notify: Boolean, autoStart: Boolean) {
        synchronized(listenerSet) {
            ...
            downloads.forEach {
                uiHandler.post {
                    when (it.status) {
                        ...
                        Status.FAILED -> {
                            listener.onError(it, it.lsDownloaderError, null)
                        }
                        Status.CANCELLED -> {
    ...
    override fun removeListener(listener: LSDownloaderListener) {
        synchronized(listenerSet) {
    ...
    override fun clearListener() {
        listenerCoordinator.clearListener()
    }
    ...
    override fun getFetchGroup(id: Int): LSDownloaderGroup {
        return groupInfoProvider.getGroupInfo(id, Reason.OBSERVER_ATTACHED)
    }

    override fun addFetchObserversForDownload(downloadId: Int, vararg fetchObservers: LSDownloaderObserver<Download>) {
        listenerCoordinator.addFetchObserversForDownload(downloadId, *fetchObservers)
    }

    override fun removeFetchObserversForDownload(downloadId: Int, vararg fetchObservers: LSDownloaderObserver<Download>) {
