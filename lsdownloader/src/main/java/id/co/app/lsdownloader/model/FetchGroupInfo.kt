...
import id.co.app.lsdownloader.utils.calculateProgress

class FetchGroupInfo(override val id: Int = 0,
                     override val namespace: String): LSDownloaderGroup {

    private val observerSet = mutableSetOf<LSDownloaderGroupObserver>()
    ...
    fun update(downloads: List<Download>, triggerDownload: Download?, reason: Reason) {
        ...
        if (reason != Reason.DOWNLOAD_BLOCK_UPDATED) {
            LSDownloaderModulesBuilder.mainUIHandler.post {
    ...
    override val observers: Set<LSDownloaderObserver<List<Download>>>
        get() {
    ...
    override fun addFetchGroupObservers(vararg fetchGroupObservers: LSDownloaderGroupObserver) {
        synchronized(observerSet) {
            val newFetchGroupObservers = fetchGroupObservers.distinct()
            val addedObservers = mutableListOf<LSDownloaderGroupObserver>()
    ...
    override fun removeFetchGroupObservers(vararg fetchGroupObservers: LSDownloaderGroupObserver) {
        synchronized(observerSet) {
