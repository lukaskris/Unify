            ...
            if (!interrupted && !terminated) {
                ...
                if (retryOnNetworkGain) {
                    ...
                    if (disconnectDetected) {
                        error = LSDownloaderError.NO_NETWORK_CONNECTION
                    }
                }
                downloadInfo.downloaded = downloaded
                downloadInfo.total = total
                downloadInfo.lsDownloaderError = error
                if (!terminated && !interrupted) {
                    downloadInfo.etaInMilliSeconds = -1
                    downloadInfo.downloadedBytesPerSecond = -1
                    delegate?.onError(download = downloadInfo, LSDownloaderError = error, throwable = e)
                }
            }
        } finally {
            try {
